...
class ImageRecorderNode(Node):
    ...
    def __init__(self):
        ...
        self.image_topic = "/image/compressed"
        ...
        self.time_synchronizer = message_filters.ApproximateTimeSynchronizer(
            [self.image_sub, self.joy_xy_sub], queue_size=15, slop=0.1
        )
    ...
    def synchronized_callback(self, image_msg: CompressedImage, joy_msg: Float32MultiArray):
        ...
        # --- Add check for non-zero joystick values ---
        if joy_x == 0.0 and joy_y == 0.0:
            self.get_logger().debug("Skipping image: Both joystick values are zero.")
        ...
        joy_x_str = f"{joy_x:.3f}".replace('.', 'p') # Replace decimal point
        joy_y_str = f"{joy_y:.3f}".replace('.', 'p')
